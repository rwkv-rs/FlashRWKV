...
void pretrain_recurrent_fp32io16_backward_large_kernel(
    int num_heads,
    const int* __restrict__ sequence_chunk_offsets,
    const int* __restrict__ chunk_token_starts,
    const int* __restrict__ chunk_token_ends,
...
void launch_pretrain_recurrent_fp32io16_backward(
  ...
  if constexpr (HeadSize == kHeadSize) {
    ...
    pretrain_recurrent_fp32io16_backward_large_kernel<HeadSize, io_t>
        <<<dim3(num_heads, num_sequences), HeadSize, 0, stream>>>(
            num_heads,
            sequence_chunk_offsets.data_ptr<int>(),
            chunk_token_starts.data_ptr<int>(),
...
void pretrain_recurrent_fp32io16_backward_cuda_impl(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        const auto launch = [&]<int HeadSize>() {
          launch_pretrain_recurrent_fp32io16_backward<HeadSize, scalar_t>(
              num_sequences,
              num_heads,
...
void pretrain_recurrent_fp32io16_backward_cuda(
    ...
    torch::Tensor grad_initial_state,
    double scale) {
  pretrain_recurrent_fp32io16_backward_cuda_impl(
