...
void launch_pretrain_recurrent_fp32io16_forward(
    ...
    float scale,
    cudaStream_t stream) {
  pretrain_recurrent_fp32io16_forward_kernel<HeadSize, io_t>
...
void pretrain_recurrent_fp32io16_forward_cuda_impl(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND2(
      ...
      [&] {
        switch (state.size(2)) {
          case 64:
            launch_pretrain_recurrent_fp32io16_forward<64, scalar_t>(
            ...
            break;
          case 128:
            launch_pretrain_recurrent_fp32io16_forward<128, scalar_t>(
                num_sequences, num_heads, sequence_chunk_offsets,
                chunk_token_starts, chunk_token_ends, state, r, decay,
                k, v, a, b, output, boundary, state_dot_a,
                static_cast<float>(scale), stream);
            break;
          case 256:
            launch_pretrain_recurrent_fp32io16_forward<256, scalar_t>(
                num_sequences, num_heads, sequence_chunk_offsets,
                chunk_token_starts, chunk_token_ends, state, r, decay,
                k, v, a, b, output, boundary, state_dot_a,
                static_cast<float>(scale), stream);
...
void pretrain_recurrent_fp32io16_forward_cuda(
    ...
    torch::Tensor state_dot_a,
    double scale) {
  pretrain_recurrent_fp32io16_forward_cuda_impl(
