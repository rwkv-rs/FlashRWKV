...
std::vector<torch::Tensor> statetune_tmix_mix6_forward_cuda(
...
std::vector<torch::Tensor> statetune_tmix_mix6_forward(
    torch::Tensor x, torch::Tensor initial_shift, torch::Tensor x_r,
    torch::Tensor x_w, torch::Tensor x_k, torch::Tensor x_v,
    torch::Tensor x_a, torch::Tensor x_g) {
  check_bf16_cuda(x, "x");
  ...
  return statetune_tmix_mix6_forward_cuda(
      x, initial_shift, x_r, x_w, x_k, x_v, x_a, x_g);
}

void register_statetune_tmix_mix6_forward_bindings(py::module_& module) {
  module.def(
      "statetune_tmix_mix6_forward", &statetune_tmix_mix6_forward,
