...
void check_common(
    ...
    TORCH_CHECK(
        k.dim() == 3 && k.size(0) > 0 && k.size(1) > 0 && k.size(2) > 0,
        "k must have non-empty shape [B, T, C]");
    TORCH_CHECK(a.sizes() == k.sizes(), "a shape mismatch");
    const int64_t c = k.size(2);
    check_vec(k_k, c, "k_k");
    check_vec(k_a, c, "k_a");
    TORCH_CHECK(k_k.device() == k.device(), "k_k must be on the same device as k");
    TORCH_CHECK(a.device() == k.device(), "a must be on the same device as k");
    TORCH_CHECK(k_a.device() == k.device(), "k_a must be on the same device as k");
...
std::vector<torch::Tensor> tmix_kk_pre_v5_backward(
    ...
    TORCH_CHECK(grad_new_k.device() == k.device(), "grad_new_k must be on the same device as k");
    TORCH_CHECK(grad_neg_kk.device() == k.device(), "grad_neg_kk must be on the same device as k");
    TORCH_CHECK(grad_kka.device() == k.device(), "grad_kka must be on the same device as k");
    TORCH_CHECK(inv_d.device() == k.device(), "inv_d must be on the same device as k");
