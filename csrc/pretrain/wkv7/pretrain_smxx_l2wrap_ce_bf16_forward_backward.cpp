...
int64_t check_logits_targets(const torch::Tensor& logits, const torch::Tensor& targets) {
    ...
    TORCH_CHECK(targets.device() == logits.device(), "targets must be on the same device as logits");
    const int64_t vocab = logits.size(-1);
    TORCH_CHECK(logits.numel() / vocab > 0, "logits must contain at least one row");
...
TORCH_LIBRARY(rwkv7_l2wrap_ce_bf16_v2, m) {
