...
// Each thread owns one channel and a row tile. This keeps v0 in a register,
// writes the BF16 grad_v12 directly, and reduces grad_v0 through FP32 partials.
...
std::vector<torch::Tensor> tmix_vres_gate_v3_backward_cuda(
    ...
    const dim3 blocks(
        static_cast<unsigned int>(ceil_div(c_size, static_cast<int64_t>(kBackwardThreads))),
        static_cast<unsigned int>(num_tiles));
    auto stream = at::cuda::getCurrentCUDAStream();

    vres_gate_v3_backward_tiled_kernel<<<blocks, kBackwardThreads, 0, stream>>>(
    ...
    const int reduce_blocks = static_cast<int>(ceil_div(c_size, static_cast<int64_t>(kBackwardThreads)));
    vres_gate_v3_reduce_v0_kernel<<<reduce_blocks, kBackwardThreads, 0, stream>>>(
        partial_v0.data_ptr<float>(),
