...
torch::Tensor tmix_a_gate_forward_cuda(torch::Tensor a0, torch::Tensor a12) {
    ...
    int blocks = static_cast<int>(ceil_div(total, static_cast<int64_t>(kThreads)));
    auto stream = at::cuda::getCurrentCUDAStream();
    if (is_power_of_two(c_size)) {
        a_gate_forward_pow2c_kernel<<<blocks, kThreads, 0, stream>>>(
        ...
        a_gate_forward_kernel<<<blocks, kThreads, 0, stream>>>(
            a0.data_ptr<at::BFloat16>(),
            a12.data_ptr<at::BFloat16>(),
            out.data_ptr<at::BFloat16>(),
            total,
...
std::vector<torch::Tensor> tmix_a_gate_backward_cuda(
    ...
    dim3 blocks(
        static_cast<unsigned int>(ceil_div(c_size, static_cast<int64_t>(kThreads))),
        static_cast<unsigned int>(num_tiles));
    auto stream = at::cuda::getCurrentCUDAStream();
    a_gate_backward_full_kernel<<<blocks, kThreads, 0, stream>>>(
        ...
        c_size);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
    int reduce_blocks = static_cast<int>(ceil_div(c_size, static_cast<int64_t>(kThreads)));
    a_gate_reduce_a0_kernel<<<reduce_blocks, kThreads, 0, stream>>>(
        partial_a0.data_ptr<float>(),
