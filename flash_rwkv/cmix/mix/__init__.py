...
def pretrain_cmix_bf16(
    ...
    for name, tensor in zip(("x", "x_k", "key_weight", "value_weight"), tensors, strict=True):
        if not isinstance(tensor, torch.Tensor) or tensor.dtype != torch.bfloat16 or not tensor.is_cuda or not tensor.is_contiguous():
    ...
    if key_weight.shape != (4 * x.shape[-1], x.shape[-1]) or value_weight.shape != (x.shape[-1], 4 * x.shape[-1]):
